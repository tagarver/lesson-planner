...
def generate_weekly_poster(week):
    ...
    font_title = ImageFont.load_default()  # Use default; add arial.ttf if available
    ...
    for student in students:
        draw.text((20, y), f"{student[1]}'s Week:", fill=colors["text"], font=font_text)
        y += 30
        student_plans = [p for p in plans if p[2] == student[0]]
        for plan in student_plans:
            draw.text((40, y), f"{plan[3]}: {plan[4]} - {plan[5].split('Objectives: ')[1].split('\\n')[0]} (Mastery: {plan[6]})", fill=colors["activity"], font=font_activity)
            y += 20
        y += 20
